...
export function ModalRoot() {
    ...
    const sellPositionDialog = (exitpositondata) => {
        ...
        {exitpositondata.showSymbolModal && (<motion.div variants={backdrop} initial="hidden" animate="visible" exit="hidden" className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end md:items-center justify-center" onClick={exitpositondata.setShowSymbolModal(false)}>
            <motion.div variants={sheet} initial="hidden" animate="visible" exit="exit" onClick={(e) => { e.stopPropagation(); exitpositondata.setSelectedSymbol(exitpositondata.positionSymbol); }} className="bg-white w-full md:w-[400px] rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden">
              {/* Header */}
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-slate-50/50">
                ...
                <button onClick={() => { exitpositondata.setShowSymbolModal(false); exitpositondata.setIsVisible(false); }} className="p-2 hover:bg-gray-200 rounded-full transition-colors">
              ...
              <div className="p-5 space-y-5 max-h-[85vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                      {['Market', 'Limit'].map((t) => (<button key={t} onClick={() => exitpositondata.setOrderType(t)} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${exitpositondata.orderType === t ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400'}`}>
                          {t}
                        </button>))}
                    </div>
                  </div>
                ...
                {/* */}  <div className="space-y-2">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                      {['MARGIN', 'CNC'].map((p) => (<button key={p} onClick={() => exitpositondata.setProductMode(p)} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${exitpositondata.productMode === p ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400'}`}>
                          {p}
                        </button>))}
                    </div>
                  </div>
                ...
                <div className="space-y-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono  font-bold text-gray-800">{exitpositondata.positionQty}</span>
                  </div>
                  <input type="range" min="0" step={exitpositondata.lotSize} max={exitpositondata.boughtQty} value={exitpositondata.positionQty} onChange={(e) => exitpositondata.setPositionQty(Number(e.target.value))} className="w-full h-1.5 bg-gray-200 rounded-lg mobile-margin-qty appearance-none cursor-pointer accent-red-500"/>
                  <div className="flex justify-between text-[10px] text-gray-400 font-medium">
                    <span>0</span>
                    <span>Max: {exitpositondata.boughtQty}</span>
                  </div>
                </div>
                ...
                <div className={`space-y-3 p-4 rounded-xl border transition-all ${exitpositondata.orderType === 'Market' ? 'opacity-40 bg-gray-100' : 'bg-gray-50 border-gray-100'}`}>
                  ...
                  <input type="range" disabled={exitpositondata.orderType === 'Market'} min="0" step="0.05" max={1000} value={exitpositondata.positionPrice} onChange={(e) => exitpositondata.setPositionPrice(Number(e.target.value))} className="w-full h-1.5 bg-gray-200 mobile-margin-price rounded-lg appearance-none cursor-pointer accent-red-500"/>
                </div>

                {/* Scheduled Checkbox / Toggle */}
                <div onClick={() => exitpositondata.setIsScheduled(!exitpositondata.isScheduled)} className="flex items-center justify-between p-4 bg-blue-50/50 rounded-xl border border-blue-100 cursor-pointer transition-colors hover:bg-blue-50">
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => { exitpositondata.setShowSymbolModal(false); exitpositondata.setIsVisible(false); }} className="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-bold text-sm hover:bg-gray-200 transition-colors">
                    Cancel
                  </button>
                  <button onClick={exitpositondata.dispatchSellSelected} disabled={exitpositondata.positionQty <= 0} className={`flex-[2] py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm shadow-lg shadow-red-200 transition-all active:scale-95
    ...
    const razorPayDialog = (razorpayorder, setShowModal) => {
        ...
        return (<AnimatePresence>
      {razorpayorder?.show && razorReceipt && (<motion.div variants={backdrop} initial="hidden" animate="visible" exit="hidden" className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end md:items-center justify-center" onClick={() => setShowModal(false)}>
          <motion.div variants={sheet} initial="hidden" animate="visible" exit="exit" onClick={(e) => { e.stopPropagation(); /*razorpayorder.show = false;*/ }} className="bg-white w-full md:w-[380px] rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden">
            {/* Header */}
            ...
            <div className="p-4 space-y-4 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`flex items-center gap-1 font-semibold ${isSuccess ? "text-green-600" : "text-orange-500"}`}>
    ...
    return (<>
      ...
      {(modal.payload !== undefined && modal.payload !== null) && modal.payload?.modalType === 'exitposition' ? (sellPositionDialog(modal.payload)) : ((modal.payload !== undefined && modal.payload !== null) && modal.payload?.modalType === 'razorpayorder' ? (razorPayDialog(modal.payload ?? modalpayload.payload, dialogRazor)) : (<div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
  {/* Backdrop */}
  ...
  <div className="relative w-full max-w-sm sm:max-w-md bg-white rounded-xl shadow-xl overflow-hidden">
    {/* Header */}
    <div className={`px-4 py-2 sm:px-6 sm:py-3 border-b ${isError ? "bg-red-50" : "bg-gray-50"}`}>
    ...
    <div className="flex items-center justify-end gap-2 px-4 py-2 sm:px-6 sm:py-3 border-t">
      <button onClick={close} className="px-3 py-1.5 rounded-md text-sm font-medium bg-gray-100 hover:bg-gray-200">
        Close
      </button>

      <button onClick={confirm} className={`px-3 py-1.5 rounded-md text-sm font-semibold ${isError
